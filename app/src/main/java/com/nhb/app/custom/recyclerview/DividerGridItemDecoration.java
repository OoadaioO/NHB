...
public class DividerGridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void setDivider(Context context, int drawable) {
    ...
    public void onDraw(Canvas c, RecyclerView parent, State state) {
        ...
        if (adapter instanceof NHBRecyclerAdapter) {
            for (int i = 0; i < adapter.getItemCount(); i++) {
                if (adapter.getItemViewType(i) == NHBRecyclerAdapter.VIEW_HEADER) {
                    headerCount++;
                } else if (adapter.getItemViewType(i) == NHBRecyclerAdapter.VIEW_FOOTER) {
                    footerCount--;
                }
            }
        }
    ...
    private boolean isLastColum(RecyclerView parent, int pos, int spanCount, int childCount) {
        pos -= headerCount;
        childCount = childCount - headerCount - footerCount;
    ...
    public void getItemOffsets(Rect outRect, int itemPosition, RecyclerView parent) {
        ...
        if (isLastRaw && isLastColum) {
            outRect.set(0, 0, 0, 0); // 最右下角一个不用绘制
        } else if (isLastRaw) {
